...
COUNTRY_CONFIG_PATH = os.path.join(os.path.dirname(__file__), "config", "country_documents.json")
COUNTRY_DOCUMENTS = {}
try:
    with open(COUNTRY_CONFIG_PATH, "r") as f:
        COUNTRY_DOCUMENTS = json.load(f)
    print(f"✓ Loaded {len(COUNTRY_DOCUMENTS.get('countries', []))} countries from config")
except Exception as e:
...
def validate_document_for_country(country_code, document_id):
    ...
    all_docs = (
        country.get("identityDocuments", []) + country.get("addressDocuments", [])
    )
...
def create_application():
    ...
    try:
        ...
        audit(app_id, "application_created", f"country={country_code} doc_id={document_id} doc_type={doc_type}")
